...
def Bayes_classifier(number_sick,number_healthy,Probability_sick,Probability_healthy,List_param,List_other_param,List_Rez):
    ...
    Keys_other_param=('List_Pol')
    Keys_all=('List_Age','List_Pol','List_Har','List_VG','List_YH','List_YS','List_Card','List_MaxP','List_StepTest','List_Sklon','List_Sosyd','List_Sps')
    Result_Vocabluary={'List_Age':0,'List_Har':0,'List_VG':0,'List_VG':0,'List_YH':0,'List_YS':0,'List_Card':0,'List_MaxP':0,'List_StepTest':0,'List_Sklon':0,'List_Sosyd':0,'List_Sps':0,'List_Pol':0}
    ...
    #подсчет параметра Пол  и сохранение в словарь
    a=calculate_other_parameters(List_other_param)
    Result_Vocabluary['List_Pol']=a

    #Список названий параметров
    List_parametrname=('ВОЗРАСТ','ПОЛ','ХАРАКТЕР БОЛИ В СЕРДЦЕ','ВЕРХНЯЯ ГРАНИЦА ДАВЛЕНИЯ','УРОВЕНЬ ХОЛЕСТЕРИНА','УРОВЕНЬ САХАРА','РЕЗУЛЬТАТЫ КАРДИОГРАММЫ В СОСТОЯНИИ ПОКОЯ','МАКСИМАЛЬНАЯ ВЕЛИЧИНА ПУЛЬСА','ДАННЫЕ СТЕП-ТЕСТА','СКЛОНЕНИЕ ДАННЫХ ПО СТЕП-ТЕСТУ','КОЛИЧЕСТВО ОСНОВНЫХ СОСУДОВ','СТЕПЕНЬ ПОВРЕЖДЕНИЯ СОСУДОВ')
